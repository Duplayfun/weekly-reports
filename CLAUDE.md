...
3. **Amazon 美欧周报**：
   ...
   - **热卖商品名称必须附带可点击的 Amazon 链接**，跳转到对应站点搜索该产品
   - 链接格式：`https://www.amazon.com/s?k=产品关键词`（美国站）或 `https://www.amazon.de/s?k=产品关键词`（德国站）等
4. **宁可数据少，宁可报告短，也不能编造**

### 周报覆盖范围

- **正式周报**：覆盖周一至周六的数据
- **周中预览**：用户周中触发时，覆盖周一至当日数据，标题标注"截至 X月X日"，不是正式版
- 正式周报通常在周六生成
- 禁止以"周三特别版"等命名发布只有几天数据的报告作为正式周报

### 生成流程

每次生成周报前，必须完成以下核实：

1. 搜索采集原始数据（日期范围：周一到生成日）
